...
import { countDeepdiveStaleSections } from "@/lib/manual-deepdive-sync"
import { WARNING_TEXT } from "@/lib/semantic-styles"
import { Button } from "@/components/ui/button"
import { cn } from "@/lib/utils"

/** 深掘り回答変更に伴うマニュアル要確認（フローウィザードは出さない） */
export function DeepdiveStaleBanner({
  project,
  onShowStaleSections,
  isMobile,
}: {
  ...
  const count = countDeepdiveStaleSections(project)
      ...
      <div className={cn("flex gap-3", isMobile ? "flex-col" : "items-start justify-between")}>
        <div className="flex min-w-0 items-start gap-2">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold">深掘り回答の変更が {count} 件あります</p>
            <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">
              該当セクションを確認し、必要なら「AI再生成」してください。内容が問題なければ「深掘りを反映済み」でそのまま残せます。
            </p>
          </div>
        </div>
        <Button
