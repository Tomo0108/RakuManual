 ...
 * docs/マニュアルPPTX必須要件.md §5 および参考資料の実物から抽出。
 * ※ 参考資料の製品名・システム名は文体の手本用であり、生成本文へ持ち込まない。
 */

export const PROMPT_VERSION = "2026-08-23-ros-style-v5"

/** LLM への共通ロール定義（system の冒頭） */
export const MANUAL_AUTHOR_ROLE = `あなたは日本の社内業務マニュアル（営業・オペレーション向け）の専門ライターです。
業務システム・社内アプリの操作手順を、現場担当者がそのまま実行できる粒度で書きます。
推測で埋めず、根拠が無い箇所は needsConfirm:true にしてください。
ヒアリング・フロー・深掘り・ユーザー指示に現れない製品名・システム名・アプリ名を、参考例やプロンプトの例示から勝手に本文へ入れないでください。`
...
export const WRITING_RULES = [
  ...
  "1中項目=1操作単位（参考: 1.1 商品タイプ選択）。長い同一操作だけ 2.2.1（1/4）のように分冊番号で示す",
  "システム名は【】で括ることが多い（例:【営業】発注前確認アプリ）。利用システム欄と整合させる。名前は入力コンテキストにあるものだけ使う",
  "操作手順（type:step）は敬体で「〜してください。」で終える。文末を「〜すること」「〜しないこと」で終えない",
  "禁止事項は「〜しないでください。」、完了確認は「〜になっていることを確認してください。」と書く",
  "参考例・スキーマ例示の固有名詞（業務名・アプリ名・製品名）は文体・構造の手本のみ。対象業務の入力に無い語は出力しない",
] as const
...
export const FLOW_RULES = [
  ...
  "process/decision の sectionNumber は参考資料粒度: 大項目 N → 中項目 N.M（1操作=原則1スライド）→ 小項目 N.M.K は同一操作の分冊・分岐枝のみ（例: 1.1, 2.1, 2.2.1〜2.2.4）。無関係な手順を全部 1.1.n にしない",
